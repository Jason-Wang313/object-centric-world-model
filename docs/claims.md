...
- Controlled object-centric scenes show selected-tail binding failure where object score can increase while real utility stagnates or falls; the good-scene negative control avoids the same collapse, and dense plus extreme object-count OOD variants reproduce the corrupted-setting failure.
- Identity consistency, hidden-property calibration, targeted probing, observable-only repair, pilot-label calibration, and combined repair improve selected utility in the controlled synthetic setting, including paired seed gains, bootstrap confidence checks, repair ablations, seed-block robustness, score-noise sensitivity, noisy-probe reliability stress, dense-object OOD stress, extreme 10/12-object stress, held-out domain-randomized stress, counterfactual target-swap stress, held-out pilot-label and leave-one-failure-out pilot calibration, toy model-family proxy diagnostics, and a high-N stress panel.
- A CPU NumPy semi-learned object-centric model improves property, identity-alignment, and transition prediction over simple baselines on generated trajectories, with feature ablations and held-out dense/occluded/crossing synthetic domain-shift checks showing object information matters.
