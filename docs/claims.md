...
- Identity consistency, hidden-property calibration, targeted probing, and combined repair improve selected utility in the controlled synthetic setting, including paired seed gains, repair ablations, seed-block robustness, and a high-N stress panel.
...
The executable audit writes the machine-readable version to `results/claims_status.json`. Core claims are `strongly_supported` only if numeric thresholds in the generated claim-strength record pass, required artifacts verify, and paper text avoids supported overclaims.
