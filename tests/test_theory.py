...
from object_binding_tail_audit.theory import (
    anti_aligned_score_check,
    exact_score_tail_expected_utility,
    exact_binary_expected_success,
    monte_carlo_score_tail_selection,
    oracle_score_check,
    selected_tie_groups,
)
...
def test_exact_law_real_utility_with_ties():
    ...
    assert math.isclose(exact_score_tail_expected_utility(utilities, scores, n=2), expected)
...
def test_binary_constant_oracle_anti_aligned_and_monte_carlo():
    ...
    assert math.isclose(exact_score_tail_expected_utility([0.42, 0.42, 0.42], [3, 2, 1], n=16), 0.42)
    assert oracle_score_check(utilities, n=8)["passes"]
    assert anti_aligned_score_check(utilities, n=8)["passes"]
    exact = exact_score_tail_expected_utility(utilities, [0.0, 0.2, 0.9, 1.0], n=4)
    mc = monte_carlo_score_tail_selection(utilities, [0.0, 0.2, 0.9, 1.0], n=4, trials=25_000, seed=3)
