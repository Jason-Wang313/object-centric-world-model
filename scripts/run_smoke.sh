...
pick_python() {
  for candidate in python.exe python python3 py.exe py; do
    if command -v "$candidate" >/dev/null 2>&1 && "$candidate" -c "import numpy, pandas, matplotlib" >/dev/null 2>&1; then
      printf '%s\n' "$candidate"
      return 0
    fi
  done
...
PYTHON_BIN="$(pick_python)" || {
  echo "No Python with numpy, pandas, and matplotlib found on PATH" >&2
  exit 1
}
...
"$PYTHON_BIN" experiments/run_experiments.py --mode smoke --root "$PY_ROOT"
